...
class P2PTransferNotifier extends StateNotifier<P2PTransferState> {
  ...
  /// Adds the current transfer's recipient to the favourites list.
  /// Called from the success dialogue when the user toggles "Add to favourites".
  void addFavourite() {
    final recipient = state.transferData.recipient;
    if (recipient == null) return;
    final alreadyFavourited = state.favouriteContacts.any(
      (c) => c.accountNumber == recipient.accountNumber,
    );
    if (alreadyFavourited) return;
    final newFavourite = RecentContact(
    ...
    state = state.copyWith(
      favouriteContacts: [newFavourite, ...state.favouriteContacts],
    );
  }
